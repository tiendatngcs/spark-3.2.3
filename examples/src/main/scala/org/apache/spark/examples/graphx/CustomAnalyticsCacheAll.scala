 ...
 */
object CustomAnalyticsCacheAll {
  ...
  def main(args: Array[String]): Unit = {
    ...
    taskType match {
      case "pagerank" =>
        ...
        val graph = partitionStrategy.foldLeft(unpartitionedGraph)(_.partitionBy(_).cache()).cache()
        ...
        }).cache().vertices.cache()

        println(s"GRAPHX: Total rank: ${pr.map(_._2).cache().reduce(_ + _)}")
      ...
      case "cc" =>
        ...
        println(
          s"Components: ${cc.vertices.cache().map {
            case (vid, data) => data
          }.cache().distinct().cache()}")
        sc.stop()
      ...
      case "triangles" =>
        ...
        val triangles = TriangleCount.run(graph).cache()
        val triangleTypes = triangles.vertices.cache().map {

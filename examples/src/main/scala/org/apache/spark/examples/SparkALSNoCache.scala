...
object SparkALSNoCache {
  ...
  def main(args: Array[String]): Unit = {
    ...
    options.toArray match {
      case Array(m, u, f, iters, slices_, appName) =>
        M = m.getOrElse("100").toInt
        U = u.getOrElse("500").toInt
        F = f.getOrElse("10").toInt
        ITERATIONS = iters.getOrElse("5").toInt
        slices = slices_.getOrElse("2").toInt
        AppName = appName.getOrElse("SparkALS")
      case _ =>
        System.err.println("Usage: SparkALS [M] [U] [F] [iters] [partitions] <AppName>")
        System.exit(1)
    }
